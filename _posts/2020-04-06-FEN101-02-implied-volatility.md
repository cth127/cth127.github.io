...
for k=1:100
    Z=randn(1,Nt);
    for i=1:Nt
        S(i+1) = S(i) * exp( ( r - 1/2 * vol^2 ) * dt + vol * Z( i ) * sqrt( dt ));
    end
    x(k) = S(end);
    plot(t, S, '-');
end
...
그 결과 KOSPI200의 2020년 5월 14일 기댓값은 228.0926로 계산된다. 이렇게 생성한 주가 경로를 플롯으로 그리면 다음과 같다.
...
for k=1:201
    Z=randn(1,Nt);
    for i=1:Nt
        S(i+1) = S(i) * exp( ( r - 1/2 * vol(k)^2 ) * dt + vol(k) * Z( i ) * sqrt( dt ));
    end
    x(k) = S(end);
    plot(t, S, '-');
end
