...
무슨 의미인지 모르겠는 부분도 많지만 어쨌든 모종의 rule-based 방식을 통해 좋은 성능의 w.t.를 이뤘다고 spaCy는 주장하고 있다. 
...
특기할 사항으로는 따옴표나 괄호와 같은 기호가 앞, 뒤 단어에 붙어있는 점, 그럼에도 '(iii)'나 '$44.75'와 같은 숫자 표현은 붙어서 유지되고 있다는 점이다. 한편 금융쪽 문서에서 자주 등장하는 단어인 'Effective Time'이라든지, 띄어쓰기로 이뤄진 회사 이름 등을 하나의 토큰으로 인식할 수 있는 방법이 있을까 찾아보았지만, 앞서 서술하였듯 토큰화 알고리즘의 첫 순서가 띄어쓰기 기반 토큰화이고, 그 다음이 예외사항을 찾는 것이기 때문에, 이미 토큰화가 이뤄진 'Effective'와 'Time'을 'Effective Time'으로 토큰화 시키는 `tokenizer` 함수 내 기능은 없는 것 같았다. (있다면 메일로 알려주시길 부탁드립니다.) 따라서 원래 문서에서 합칠 단어를 'Effective Time' -> 'EffectiveTime' 식으로 바꾸는 게 현실적인 대안으로 보인다.
