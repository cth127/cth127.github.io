...
무슨 의미인지 모르겠는 부분도 많지만 어쨌든 모종의 rule-based 방식을 통해 좋은 성능의 w.t.를 이뤘다고 spaCy는 주장하고 있다. 이외에 2번에서 사용할 수 있는 예외사항을 직접 입력할 수도 있으니 자세한 사항은 위 링크를 참조하면 된다. (예컨대 금융쪽 문서에서 자주 등장하는 단어인 'Effective Time'을 예외로 설정함으로써 괜히 '효과적인 시간'으로 임베딩되는 것을 막을 수 있다.)

spaCy `tokenizer`는 `Doc`으로부터 기본 입력으로 `vocab`을 받는데, 이는 토큰화를 할 때 사용하는 단어들의 목록이다. 이외 변수 중 특기할만한 것은 dict형을 입력으로 받는 `rules`로, 알고리즘에서 2번에 해당하는 예외를 설정할 수 있다. (이후에 `add_special_case` 함수를 통해 규칙을 추가할 수도 있다.) 이외 `prefix_search`, `suffix_search`, `infix_finditer`, `token_match` 등의 변수는 3~7번에 해당하는 내용인데 필자가 이해하지 못하는 고로 다루지는 않는다. 이렇게 입력을 받은 `tokenizer`는 `Tokenizer`라는 객체롤 돌려준다.
