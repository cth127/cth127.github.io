...
t.o.i.에서도 마찬사지로 issuer가 주식을 매수할 기간을 정한다. 따라서 우리가 얻어야 할 정보구조는 누가(issuer), 무엇을(target), 언제부터(start date), 언제까지(end date), 얼마의 범위에(amount-amount), 얼마만큼(total) + announcement date, etc.가 될 것이다. 그 실제 텍스트는 다음과 같다.
