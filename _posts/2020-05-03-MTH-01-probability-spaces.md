...
을 만족하면 $\mathcal{S}$를 'semi-algebra'라 한다.
...
을 만족하면 $\mathcal{A}$를 'algebra'라 한다.
...
1. Stieltjes measure function : 함수 $F:\mathbb{R}\rightarrow[0,\infty)$가 다음 두 조건,

- $F\ is\ nondecreasing$

- $F\ is\ right\ continuous,\ i.e.\ x_i\to x^+ \Rightarrow \lim_{i\to\infty}F(x_i)=F(x)$  

을 만족하면 F를 'Stieltjes measure function'이라 한다.

1. 'Lebeque measure' : 임의의 Stieltjes measure function $F$에 대해 measurable space ($mathbb{R}, \mathcal{R}$)에서 정의된 measure $\mu\ s.t.\ \mu((a,b]) = F(b) - F(a)$가 유일하게 존재한다. 이때 $F(x) = x$이면 $\mu((a,b]) = b - a$를 'Lebeque measure'라 한다.
