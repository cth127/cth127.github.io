...
그리고 이후엔 IE에 유용허게 이용할 수 있는 정규표현식를 다뤄보겠다. 이 과정을 시작하기에 앞서 다음 글에서는 spaCy와 NLTK 라이브러리의 기본적인 구조를 살펴보겠다.
