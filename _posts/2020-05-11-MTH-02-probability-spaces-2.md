...
- Definition : semi-algebra $\mathcal{S}$에서 정의된 함수 $\mu$와  $\bar{\mathcal{S}}$ : algebra generated by $\mathcal{S}$, 그리고 집합 $A \in \bar{\mathcal{S}}$에 대해 $A = \bigcup_{n=1}^{n}S_i \text{, where S_i \in \mathcal{S} and S_i's are pairwise disjoint$이면, $\bar{\mathcal{S}}$에서 정의된 함수 $\bar{\mu}(A) = \sum_{i=1}^{k} \mu(S_i)$를 함수 $\mu$의 'extended function', 혹은 'extension'이라 정의한다.

- Theorm 1.1.9.a :  semi-algebra $\mathcal{S}$에서 정의된 함수 $\mu$가 다음 세 조건,
	- $\mu(A) \geq \mu(\phi)$ = 0 \{ for } \all A \in \mathcal{S}$
	- If a set $S = \bigcup_{i=1}^{k} S_i \Rightarrow \mu(S) = \sum_{i=1}^{k} \mu(S_i)$
	- If a set $S = \bigcup_{i=1}^{\infty} S_i \Rightarrow \mu(S) \leq \sum_{i=1}^{infty} \mu(S_i)$
	을 만족시키면, $\bar{\mathcal{S}}$ : algebra generated by $\mathcal{S}$에 대해 유일한 measure on algebra, $\bar{\mu}$가 존재한다. 이때 $\bar{\mu}$는 $\mu$의 extension이다. (pf in appendix)

- Theorm 1.1.9.b : Theorm 1.1.9.a의 조건을 만족시키면서 동시에 $\bar{\mu}$ : $\sigma$-finite 하면, $\sigma(\mathcal{S})$ : the smallest $\sigma$-algebra containing $\mathcal{S}$에 대한 유일한 measure, $\tilde{\mu}$가 존재한다. 이때 $\tilde{\mu}$는 $\bar{\mu}$의 extension이다. (pf in appendix)
...
- Lemma 1.1.10 :  semi-algebra $\mathcal{S}$에서 정의된 함수 $\mu$가 다음 두 조건,
	...
	- If $A = \bigcup_{i=1}^{n}B_i \Rightarrow \bar{\mu}(A) = \sum_{i=1}^{n}\bar{\mu}(B_i)$
	- If $A \subset \bigcup_{i=1}^{n}B_i \Rightarrow \bar{\mu}(A) \leq \sum_{i=1}^{n}\bar{\mu}(B_i)$  (pf in 5p)

이 중 핵심이 되는 것은 1.1.9로, 앞서 다뤘던 여러 정의와 조건을 잘 정의해 나가면, 우리가 어렵지 않게 뭔가를 증명할 수 있는 유한의 세계의 개념, 즉 semi-algebra, algebra, mearsure on algebra 등의 개념을 통해, 뭔가를 증명해내기 어려운 무한의 세계의 개념, 즉 $\sigma$-algebra, measure으로 extend 시켜나갈 수 있음을 보이고 있다. 이러한 확장은 수학적인 '좋은 성질'을 보이기 위해 필수적인다. 

예컨대 앞으로 우리가 다루게 될 적분은 고등학교 수준의 리만 적분이 아닌, measure 차원에서 정의된 Lebesgue 적분인데, 굳이 그러는 이유는 역시 여러 '좋은 성질'이 있기 때문이다. 이때 어떤 함수에 대한 Lebesgue 적분을 하기 위해선 그 함수가 measurable한 지는 꽤 중요한 문제이다. 직관적으로 어떤 함수에 대해 measure가 적용될 수 없다면 그 길이나 범위를 측정할 수 없다는 것인데, 그 함수에 대해 무슨 수로 넓이를 구하고 면적을 구할 수 있겠는가? 

이러한 measure의 개념을 수학적으로 정교화하기 위해 무한의 세계에도 적용되도록 하는 것은 필수적인 일이었고, 우리가 상대적으로 쉽게 접근할 수 있는 유한의 세계에서, 그럴 수 없는 무한의 세계로 이어질 수 있는 다리를 놓는 일을 1.1.9가 하고 있는 것이다. 그러한 확장성을 직접 보여주는 게 Definition의 'extension' 개념, 그리고 1.1.7의 'generated' 개념이다. 공간개념은 generate되고, 함수개념은 extend되고 있다.

한편 1.1.10은 무한의 세계에 대한 1.1.9.a의 두 번째 조건이 있어야 함수 $\mu$가 measure on algebra $\bar{\mu}$로 extend될 수 있음을 보여준다. 유한의 세계만을 제약하는 1.1.10의 조건들은 아직 measure라 불릴만한 성질을 가지는 함수를 만들어내지 못함을 알 수 있다.
